...
async function _validateExpJsonAsync(exp, pkg, projectRoot, allowNetwork) {
  ...
  const expJsonExists = await (0, _config().fileExistsAsync)(_path().default.join(projectRoot, 'exp.json'));
  const appJsonExists = await (0, _config().fileExistsAsync)(_path().default.join(projectRoot, 'app.json'));
