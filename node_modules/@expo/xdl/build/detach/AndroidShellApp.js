...
function _replaceString() {
  const data = _interopRequireDefault(require("replace-string"));

  _replaceString = function () {
    return data;
  };
...
function _AndroidIcons() {
  const data = require("./AndroidIcons");
...
function ExponentTools() {
  const data = _interopRequireWildcard(require("./ExponentTools"));
...
function _AndroidIntentFilters() {
  const data = _interopRequireDefault(require("./AndroidIntentFilters"));

  _AndroidIntentFilters = function () {
    return data;
  };
...
async function runShellAppModificationsAsync(context, sdkVersion, buildMode) {
  ...
  await regexFileAsync('VERSION_NAME = null', `VERSION_NAME = "${version}"`, _path().default.join(shellPath, 'app', 'src', 'main', 'java', 'host', 'exp', 'exponent', 'generated', 'AppConstants.java'));
  ...
  if (manifest.facebookScheme) {
    await regexFileAsync('<!-- REPLACE WITH FACEBOOK SCHEME -->', `<data android:scheme="${manifest.facebookScheme}" />`, _path().default.join(shellPath, 'app', 'src', 'main', 'AndroidManifest.xml'));
  } // Add permissions
  ...
  if (privateConfig) {
    ...
    let googleMobileAdsAutoInit = privateConfig.googleMobileAdsAutoInit; // Branch
    ...
    if (googleMobileAdsAppId) {
      ...
      await deleteLinesInFileAsync(`BEGIN GOOGLE MOBILE ADS CONFIG`, `END GOOGLE MOBILE ADS CONFIG`, _path().default.join(shellPath, 'app', 'src', 'main', 'AndroidManifest.xml')); // Put user's Google Mobile Ads App ID if provided.

      await regexFileAsync('<!-- ADD GOOGLE MOBILE ADS CONFIG HERE -->', `<meta-data
      android:name="com.google.android.gms.ads.APPLICATION_ID"
      android:value="${googleMobileAdsAppId}"/>`, _path().default.join(shellPath, 'app', 'src', 'main', 'AndroidManifest.xml'));
    ...
    if (googleMobileAdsAutoInit) {
      await regexFileAsync('<meta-data android:name="com.google.android.gms.ads.DELAY_APP_MEASUREMENT_INIT" android:value="true"/>', '<meta-data android:name="com.google.android.gms.ads.DELAY_APP_MEASUREMENT_INIT" android:value="false"/>', _path().default.join(shellPath, 'app', 'src', 'main', 'AndroidManifest.xml'));
    } // Google Login
  ...
  if (buildMode === 'debug') {
    await regexFileAsync('DEVELOPMENT_URL = ""', `DEVELOPMENT_URL = "${fullManifestUrl}"`, _path().default.join(shellPath, 'app', 'src', 'main', 'java', 'host', 'exp', 'exponent', 'generated', 'DetachBuildConstants.java'));
  } // Google sign in


  await regexFileAsync(/"current_key": "(.*?)"/, `"current_key": "${googleAndroidApiKey}"`, _path().default.join(shellPath, 'app', 'google-services.json'));
  await regexFileAsync(/"certificate_hash": "(.*?)"/, `"certificate_hash": "${certificateHash}"`, _path().default.join(shellPath, 'app', 'google-services.json')); // Facebook configuration
  ...
  if (manifest.facebookAppId) {
    await regexFileAsync('<!-- ADD FACEBOOK APP ID CONFIG HERE -->', `<meta-data android:name="com.facebook.sdk.ApplicationId" android:value="${manifest.facebookAppId}"/>`, _path().default.join(shellPath, 'app', 'src', 'main', 'AndroidManifest.xml'));
  } // There's no such pattern to replace in shell apps below SDK 36,
  // so this will not have any effect on these apps.


  if (manifest.facebookDisplayName) {
    await regexFileAsync('<!-- ADD FACEBOOK APP DISPLAY NAME CONFIG HERE -->', `<meta-data android:name="com.facebook.sdk.ApplicationName" android:value="${manifest.facebookDisplayName}"/>`, _path().default.join(shellPath, 'app', 'src', 'main', 'AndroidManifest.xml'));
  } // There's no such pattern to replace in shell apps below SDK 36,
  // so this will not have any effect on these apps.


  if (manifest.facebookAutoInitEnabled) {
    await regexFileAsync('<meta-data android:name="com.facebook.sdk.AutoInitEnabled" android:value="false"/>', '<meta-data android:name="com.facebook.sdk.AutoInitEnabled" android:value="true"/>', _path().default.join(shellPath, 'app', 'src', 'main', 'AndroidManifest.xml'));
  } // There's no such pattern to replace in shell apps below SDK 36,
  // so this will not have any effect on these apps.


  if (manifest.facebookAutoLogAppEventsEnabled) {
    await regexFileAsync('<meta-data android:name="com.facebook.sdk.AutoLogAppEventsEnabled" android:value="false"/>', '<meta-data android:name="com.facebook.sdk.AutoLogAppEventsEnabled" android:value="true"/>', _path().default.join(shellPath, 'app', 'src', 'main', 'AndroidManifest.xml'));
  } // There's no such pattern to replace in shell apps below SDK 36,
  // so this will not have any effect on these apps.


  if (manifest.facebookAdvertiserIDCollectionEnabled) {
    await regexFileAsync('<meta-data android:name="com.facebook.sdk.AdvertiserIDCollectionEnabled" android:value="false"/>', '<meta-data android:name="com.facebook.sdk.AdvertiserIDCollectionEnabled" android:value="true"/>', _path().default.join(shellPath, 'app', 'src', 'main', 'AndroidManifest.xml'));
  }
}
